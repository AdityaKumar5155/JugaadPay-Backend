...
const createPersonSchema = z.object({
  ...
  debt_amount: z.coerce.number({
    error: (issue) => {
      if (issue.input === undefined) return 'Debt amount is required.';
      if (issue.code === 'invalid_type') return 'Debt amount must be a number.';
      return 'Debt amount must be a valid decimal.';
    }
  }).optional()
});
